...
function main()
    ...
    for i in 1:length(chrom_split)
        ...
        ref_dict = ParseRef("$rfdir/$rffle", chrom)

        #bim file containing valid SNPs
        vld_dict = ParseBim(parsed_args["bim_prefix"],chrom)

        #parse the summary stats
        sst_dict = ParseSumstats(ref_dict, vld_dict,parsed_args["sst_file"] , parsed_args["n_gwas"])


        ld_info = ParseLDBlock("$rfdir/", sst_dict, chrom)
        ...
        ContinuousShrinkageGibbsMCMC(
            ...
            ld_info[2],
            parsed_args["n_iter"], 
            parsed_args["n_burnin"], 
            parsed_args["thin"], 
            chrom, 
            parsed_args["out_dir"], 
            parsed_args["beta_std"], 
            parsed_args["seed"]
            )
