...
"""
struct GenInvGauss{T<:Real} <: ContinuousUnivariateDistribution
    a::T
    b::T
    p::T
    Normal{T}(a::T, b::T, p::T) where {T<:Real} = new{T}(a,b,p)
end

function GenInvGauss(a::T, b::T, p::T; check_args::Bool=true) where {T <: Real}
    @check_args GenInvGauss (a, a > zero(a)
    @check_args GenInvGauss (b, b > zero(b))
    return GenInvGauss{T}(a,b,p)
end
